function [imdsTrain, imdsVal, imdsTest, pxdsTrain, pxdsVal, pxdsTest] = helperPartitionLidarData(imds, pxds, varargin)
%helperPartitionLidarData   Partition Lidar data by randomly dividing data 
%   for training and validation based on given ratio. Rest is used for 
%   testing.
% 
%    'trainingRatio'           - The trainingRatio is used to divide the 
...
defaultTrainingRatio = 0.6;
addOptional(p,'trainingRatio', defaultTrainingRatio, @isfloat);

p.KeepUnmatched = true;
parse(p,varargin{:})

ratio = p.Results.trainingRatio;
